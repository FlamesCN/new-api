...
func testChannel(ctx context.Context, channel *model.Channel, testUserID int, testModel string, endpointType string, streamOverride *bool) testResult {
	...
	for idx, candidate := range candidates {
		result := testChannelWithModel(ctx, channel, testUserID, candidate, endpointType, isStream)
...
func testChannelWithModel(ctx context.Context, channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool) testResult {
	if ctx == nil {
...
func TestChannel(c *gin.Context) {
	...
	result := testChannel(requestCtx, channel, testUserID, testModel, endpointType, streamOverride)
...
func testChannelForHealthCheck(ctx context.Context, channel *model.Channel, testUserID int, allowDisable bool, disableThreshold int64) channelTestSummary {
	...
	tik := time.Now()
	streamOverride := shouldUseStreamForAutomaticChannelTest(channel)
	result := testChannel(ctx, channel, testUserID, "", "", common.GetPointer(streamOverride))
